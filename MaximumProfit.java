...
public class MaximumProfit {
	...
	public static void main(String[] args) {
		...
		System.out
				.println("Maximum difference is" + maxProfit(arr, arr.length));
	...
	public static int maxProfit(int price[], int n) {
		...
		for (int i = 1; i < n; i++) { // min_price is minimum price in
										// price[0..i]
			if (price[i] < min_price)
				min_price = price[i];

			// Maximum profit is maximum of: // a) previous maximum, i.e.,
			// profit[i-1] // b) (Buy, Sell) at (min_price, price[i]) and add //
